import SQLiteESMFactory from '@powersync/wa-sqlite/dist/wa-sqlite.mjs';
import * as SQLite from '@powersync/wa-sqlite';

export async function createMemoryDb(env: any): Promise<{
  db: number;
  api: SQLiteAPI;
  close: () => Promise<void>;
  ...
  const api = (SQLite as any).Factory(module) as unknown as SQLiteAPI;

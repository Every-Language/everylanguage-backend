...
function generateTestFilePaths(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    paths.push(
      `audio/${book}-${chapter.toString().padStart(2, '0')}-${fileNum}.mp3`
    );
  }
...
async function testBulkDownloadUrls(fileCount, testName) {
  ...
  try {
    ...
    console.log(
      `✅ SUCCESS in ${duration}ms (${(duration / 1000).toFixed(1)}s)`
    );
    console.log(`📊 Results:`);
    ...
    console.log(
      `   • URLs expire in: ${(data.expiresIn / 3600).toFixed(1)} hours`
    );
    ...
    if (data.failedFiles?.length > 0) {
      ...
      console.log(
        `⚠️  Error samples:`,
        Object.entries(data.errors || {}).slice(0, 2)
      );
    }
    ...
    if (duration > 120000) {
      // > 2 minutes
      console.log(
        `🚨 CRITICAL: Request took over 2 minutes (approaching timeout limit)`
      );
    }
    if (successRate < 95) {
...
async function runProgressiveTests() {
  ...
  if (successful.length > 0) {
    ...
    successful.forEach(result => {
      const timeIndicator =
        result.duration > 60000 ? '🐌' : result.duration > 30000 ? '⚠️' : '⚡';
      console.log(
        `   ${timeIndicator} ${result.fileCount} files: ${(result.duration / 1000).toFixed(1)}s (${result.avgTimePerFile.toFixed(1)}ms/file, ${result.successRate.toFixed(1)}% success)`
      );
    });
    ...
    if (maxSuccessful >= 1189) {
      ...
      console.log(
        `⚠️  You may need frontend batching for full Bible downloads`
      );
    }
  } else {
    console.log(
      `❌ All tests failed. Check your function deployment and B2 configuration.`
    );
  }
}
...
async function testWithRealData() {
  ...
  try {
    ...
    const result = await testBulkDownloadUrls(
      realPaths.length,
      'Real Data Test'
    );
    ...
    if (result.success) {
      console.log(
        `✅ Real data test successful! Your B2 integration is working.`
      );
    } else {
...
async function main() {
  try {
    ...
    const { error: pingError } = await supabase.functions.invoke(
      'get-download-urls',
      {

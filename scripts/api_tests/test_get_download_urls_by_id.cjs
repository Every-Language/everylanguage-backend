...
async function run(ids) {
  ...
  const { data, error } = await supabase.functions.invoke('get-download-urls-by-id', { body });
  if (error) {
...
if (ids.length === 0) {
  console.log('Usage: node scripts/api_tests/test_get_download_urls_by_id.cjs <id1> <id2> ...');
  process.exit(1);
}
